...
class EMT_Lib
{
	...
	{
		$text = preg_replace_callback("/\&#([0-9]+)\;/", create_function('$m', 'return EMT_Lib::_getUnicodeChar(intval($m[1]));'), $text);
		$text = preg_replace_callback("/\&#x([0-9A-F]+)\;/", create_function('$m', 'return EMT_Lib::_getUnicodeChar(hexdec($m[1]));'), $text);
		$text = preg_replace_callback("/\&([a-zA-Z0-9]+)\;/", create_function('$m', '$r = EMT_Lib::html_char_entity_to_unicode($m[1]); return $r ? $r : $m[0];'), $text);
	}
	...
	public function setup($setupmap)
	{
		...
		if(isset($setupmap[ 'map' ]) || isset($setupmap[ 'maps' ]))
		{
			...
			if(is_array($setupmap[ 'maps' ]))
			{
				...
				{
					$this->set_enable_map($map[ 'map' ], isset($map[ 'disable' ]) ? $map[ 'disable' ] : false, isset($map[ 'strict' ]) ? $map[ 'strict' ] : false);
				}
			}
